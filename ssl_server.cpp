...
//	SSL_CTX_set_options(ctx, SSL_OP_ALL | SSL_OP_NO_SSLv2 | SSL_OP_SINGLE_DH_USE);
    ...
    int siglen = RSA_private_encrypt(mdlen, (const unsigned char*)hash_string.c_str(), (unsigned char*)temp_buff2, gekas, RSA_PKCS1_PADDING);
    const char* signature= buff2hex((const unsigned char*)temp_buff2, siglen).c_str();
