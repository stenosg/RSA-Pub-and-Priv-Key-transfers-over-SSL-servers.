...
//	SSL_CTX_set_options(ctx, SSL_OP_ALL|SSL_OP_NO_SSLv2);
    ...
    char randomNumber[BUFFER_SIZE];
	//SSL_write
    int write_x;
    //int RAND_bytes(unsigned char *buf, int num);

    int randm = RAND_bytes((unsigned char*)randomNumber, 13);
  
    write_x = SSL_write(ssl, (const void*)randomNumber, BUFFER_SIZE);   

    string tempo = buff2hex((const unsigned char*)randomNumber, BUFFER_SIZE).c_str();
	...
	decrypted_key = buff2hex((const unsigned char*)temp_buff, 20).c_str();
     ...
     for(int i = 0; i < generated_key.size(); ++i)
     {
       if(generated_key[i] != buff[i])
       { 
         cout << "IN CORRECT SIGNATURE" << endl;
         break;
       }
     }
    ...
    f_name += '\0'; //without this will not be able to use bio_puts

    int bputs = BIO_puts(binfile, f_name.c_str());
	//SSL_write

    SSL_write(ssl, f_name.c_str(), BUFFER_SIZE); 
    ...
    BIO* outfile = BIO_new_file("gekas_out.txt", "w");
    ...
    while(cisse)
    {
       r_size = SSL_read(ssl, c_buffer, sizeof(c_buffer));
       printf(c_buffer);
       cout << "r_size:" << r_size << endl;
       if(r_size < BUFFER_SIZE)
       {
          for(int i = 0; i < r_size; ++i)
             l_copied[i] = c_buffer[i];
          r_lines = BIO_write(outfile, c_buffer, r_size);
          cisse = false;

       }

       if(cisse)
		r_lines = BIO_write(outfile, c_buffer, BUFFER_SIZE);
